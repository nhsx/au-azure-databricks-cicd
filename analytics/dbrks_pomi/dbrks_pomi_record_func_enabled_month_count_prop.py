...
"""
FILE:           dbrks_pomi_record_func_enabled_month_count_prop.py
DESCRIPTION:
                Databricks notebook with processing code for the NHSX Analyticus unit metric: No. and % of GP practices with appointment functionality enabled (M050)
USAGE:
                ...
...
df = df[df["Field"] == "Sys_DetCodeRec_Enbld"]
df['Value'] = df['Value'].fillna(0).astype(int)
def codedrecord_enabled(c):
  if c['Value'] == 2:
    return 1
  else:
    return 0
df['GP practice detailed coded record functionality enabled'] = df.apply(codedrecord_enabled, axis=1)
