...
"""
FILE:           dbrks_pomi_repeat_prescription_enabled_month_count_prop.py
DESCRIPTION:
                Databricks notebook with processing code for the NHSX Analyticus unit metric: No. and % of GP practices with repeat prescription functionality enabled (M051)
USAGE:
                ...
...
df = df[df["Field"] == "Sys_Presc_Enbld"]
df['Value'] = df['Value'].fillna(0).astype(int)
def prescription_enabled(c):
  if c['Value'] == 2:
    return 1
  else:
    return 0
df['GP practice repeat prescription functionality enabled'] = df.apply(prescription_enabled, axis=1)
