...
"""
FILE:           dbrks_nhs_app_uptake.py
DESCRIPTION:
                Databricks notebook to create nhs_app uptake table for dashboard
USAGE:
                ...
CONTRIBUTORS:   Abdu Nuhu, Query supplied by Oliver Jones
...
#nhs_app_uptake_registrations_day_cumsum
day_cumsum_data_dir = config_JSON['pipeline']['project']['databricks'][1]['sink_path']
day_cumsum_file_name = config_JSON['pipeline']['project']['databricks'][1]['sink_file']

print("Loaction and file name for nhs_app_uptake_registrations_day_cumsum")
print(day_cumsum_data_dir)
print(day_cumsum_file_name)
print("--------------------------------")

cumsum_data = datalake_download(CONNECTION_STRING, file_system, day_cumsum_data_dir+latestFolder, day_cumsum_file_name)
df_day_cumsum = pd.read_csv(io.BytesIO(cumsum_data))
...
#nhs_app_uptake_p9_registrations_day_cumsum
reg_p9_cumsum_data_dir = config_JSON['pipeline']['project']['databricks'][3]['sink_path']
reg_p9_cumsum_file_name = config_JSON['pipeline']['project']['databricks'][3]['sink_file']

print("Loaction and file name for nhs_app_uptake_p9_registrations_day_cumsum")
print(reg_p9_cumsum_data_dir)
print(reg_p9_cumsum_file_name)
print("--------------------------------")

reg_p9_day_cumsum_data = datalake_download(CONNECTION_STRING, file_system, reg_p9_cumsum_data_dir+latestFolder, reg_p9_cumsum_file_name)
df_reg_p9_cumsum = pd.read_csv(io.BytesIO(reg_p9_day_cumsum_data))
...
day_cumsum_df = spark.createDataFrame(df_day_cumsum)
reg_p9_day_count_df = spark.createDataFrame(df_reg_p9_day_count)
reg_p9_cumsum_df = spark.createDataFrame(df_reg_p9_cumsum)
...
day_cumsum_df2 = day_cumsum_df.withColumnRenamed("Practice code","Practice_code")\
.withColumnRenamed("Cumulative number of NHS app registrations","Cumulative_number_of_NHS_app_registrations")
...
reg_p9_cumsum_df2 = reg_p9_cumsum_df.withColumnRenamed("Practice code","Practice_code")\
.withColumnRenamed("Cumulative number of P9 NHS app registrations","Cumulative_number_of_P9_NHS_app_registrations")
...
day_cumsum_df2.createOrReplaceTempView("tv_day_cumsum")
reg_p9_day_count_df2.createOrReplaceTempView("tv_reg_p9_day_count")
reg_p9_cumsum_df2.createOrReplaceTempView("tv_reg_p9_cumsum")
...
# MAGIC SELECT
# MAGIC 
# MAGIC     a.Unique_ID as uptake_unique_id
# MAGIC     ,a.Date as uptake_date
# MAGIC     ,a.Practice_code as uptake_practice_code	
# MAGIC     ,a.Cumulative_number_of_P9_NHS_app_registrations as M0148_cumulative_number_of_p9_nhs_app_registrations
# MAGIC     ,a.Number_of_GP_registered_patients as M0148_number_of_gp_registered_patients
# MAGIC 
# MAGIC     ,b.Number_of_NHS_app_registrations as M0144_number_of_nhs_app_registrations
# MAGIC     ,c.Cumulative_number_of_NHS_app_registrations as M0143_cumulative_number_of_nhs_app_registrations
# MAGIC     ,d.Number_of_P9_NHS_app_registrations as M0146_number_of_p9_app_registrations
# MAGIC     ,e.Cumulative_number_of_P9_NHS_app_registrations as M0145_cumulative_number_of_P9_nhs_app_registrations
# MAGIC  
# MAGIC 
# MAGIC FROM
# MAGIC     tv_day_prop a
...
# MAGIC 
# MAGIC LEFT JOIN
# MAGIC     tv_day_cumsum c
# MAGIC ON
# MAGIC     a.Date=c.Date AND a.Practice_code=c.Practice_code
# MAGIC LEFT JOIN
