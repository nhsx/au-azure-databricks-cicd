...
for new_source_file in file_name_list:
  ...
  header_list = ["Location ID","Dormant (Y/N)","Care home?","Location Inspection Directorate","Location Primary Inspection Category","Location ONSPD CCG Code","Location ONSPD CCG","Provider ID","Provider Inspection Directorate","Provider Primary Inspection Category","Provider Postal Code"]
  new_date = datetime.now().strftime('%Y-%m-%d') + '/'
  #new_dataframe = pd.read_csv(io.BytesIO(new_dataset), sheet_name = 'HSCA_Active_Locations', header = 0, engine='odf', names = header_list)
  ...
  df2["run_date"] = datetime.now().strftime('%Y-%m-%d') 
  #new_dataframe = pd.read_excel(io.BytesIO(new_dataset), sheet_name = 'HSCA_Active_Locations', header = 0, engine='odf')
